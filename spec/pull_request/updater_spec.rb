...
describe PullRequest::Updater do
  ...
  let(:user) { "user" }
  let(:repo) { "repo" }
  let(:pull_request_updater) { PullRequest::Updater.new(user: user, repo: repo, sha: sha, pull_request: pull_request) }
  ...
  describe '#update_build!' do
    ...
    it "adds the build to the build_log object" do
      build_log.should_receive(:add_build!).with(sha, status, date)
      pull_request_updater.update_build!(sha, status, date)
    end
