...
describe PullRequest::Updater do
  ...
  let(:pull_request) { PullRequest::PR.new(user, repo, commit_hash, bitbucket_data) }
  let(:description) { "Pull request description" }
  let(:user) { "user" }
  let(:repo) { "repo" }
  let(:pull_request_updater) { PullRequest::Updater.new(user: user, repo: repo, sha: commit_hash, pull_request: pull_request) }
